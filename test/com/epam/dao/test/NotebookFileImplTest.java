...
public class NotebookFileImplTest {
	...
	public void addNoteTest() throws DAOException, LogicException {
		...
		Note readNote = io.readNoteFromFile(file, 0);
	...
	public void changeNoteTest() throws DAOException, LogicException{
		...
		NotebookIO io = new NotebookIO();		
		Assert.assertEquals(io.readNoteFromFile(file, 0).toString(), new NoteWithTitle(new Date("2015/03/03"), "All", "title").toString());
		file.delete();
	}
	...
	public void replaceNoteTest() throws DAOException, LogicException{
		...
		NotebookIO io = new NotebookIO();
		Assert.assertEquals(io.readNoteFromFile(file, 1).toString(), new Note(new Date("2015/01/01"), "ooo").toString());
		file.delete();	
	}
	...
	public void formatNoteTest() throws DAOException, LogicException{
		...
		NotebookIO io = new NotebookIO();
		Assert.assertEquals(io.readNoteFromFile(file, 0).toString(), new Note(new Date("2015/03/03"), "slot").toString());
